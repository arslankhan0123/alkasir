...
class Sales_model extends CI_Model {
	...
	private function _get_datatables_query()
	{
	    ...
	    //}
	     ...
	     if($sales_type == 'retail' || $sales_type=='wholesale'){
	         $this->db->where('a.sales_type',$sales_type);
	     }
	...
	public function verify_save_and_update(){
        ...
        if($count_id > 9999999999){ // 10 digits maximum
            echo "Invoice Number is too large! Maximum allowed is 9,999,999,999.";
            exit;
        }
        
        // Duplicate check for new sales
        if($command=='save' || $command == 'save_and_print'){
            $exists = $this->db->where('count_id', $count_id)
                              ->where('init_code', $init_code)
                              ->get('db_sales')
                              ->num_rows();
            if($exists > 0){
                echo "Invoice Number $init_code$count_id already exists! Please use a different number.";
                exit;
            }
        }
        ...
        if($command=='update' || $command=='update_and_print'){
            $exists = $this->db->where('count_id', $count_id)
                              ...
                              ->num_rows();
            if($exists > 0){
                echo "Invoice Number $init_code$count_id already exists in another record!";
                exit;
            }
        }
	    ...
	    if($command=='save' || $command == 'save_and_print'){//Create sales code unique if first time entry
